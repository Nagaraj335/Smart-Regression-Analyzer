...
def load_sample_data():
    """Cached function to load sample data"""
    try:
        return pd.read_csv('sample_housing_dataset.csv')
    except:
        return None
...
class SmartRegressionAnalyzer:
    def __init__(self):
        ...
        self.feature_columns = []  # Initialize as empty list instead of None
    ...
    def train_all_algorithms(self, X, y, quick_mode=True):
        ...
        # Add TensorFlow Neural Network if available
        if HAS_TENSORFLOW and not quick_mode:
            try:
                algorithm_count += 1
                progress = algorithm_count / (total_algorithms + 1)
                progress_bar.progress(progress)
                status_text.text(f"Training TensorFlow Neural Network... ({algorithm_count}/{total_algorithms + 1})")
                
                start_time = time.time()
                keras_model = self._create_keras_model(X_train_scaled.shape[1])
                ...
                if keras_model is not None:
                    ...
                    # Predictions
                    y_pred_train_keras = keras_model.predict(X_train_scaled, verbose=0).flatten()
                    y_pred_test_keras = keras_model.predict(X_test_scaled, verbose=0).flatten()
                    
                    # Calculate metrics
                    train_r2_keras = r2_score(y_train, y_pred_train_keras)
                    test_r2_keras = r2_score(y_test, y_pred_test_keras)
                    test_mae_keras = mean_absolute_error(y_test, y_pred_test_keras)
                    test_rmse_keras = np.sqrt(mean_squared_error(y_test, y_pred_test_keras))
                    overfitting_keras = train_r2_keras - test_r2_keras
                    
                    # Store results
                    self.results.append({
                        'Category': 'Neural Networks',
                        'Algorithm': 'TensorFlow Neural Network',
                        'Train R²': round(train_r2_keras, 4),
                        'Test R²': round(test_r2_keras, 4),
                        'CV R² Mean': 0.0,  # Skip CV for speed
                        'CV R² Std': 0.0,
                        'Test MAE': round(test_mae_keras, 2),
                        'Test RMSE': round(test_rmse_keras, 2),
                        'Overfitting': round(overfitting_keras, 4),
                        'Training Time (s)': round(training_time, 3)
                    })
                    
                    # Store trained model
                    self.trained_models['TensorFlow Neural Network'] = {
                        'model': keras_model,
                        'category': 'Neural Networks',
                        'scaler': scaler,
                        'y_pred_train': y_pred_train_keras,
                        'y_pred_test': y_pred_test_keras,
                        'X_train': X_train_scaled,
                        'X_test': X_test_scaled,
                        'y_train': y_train,
                        'y_test': y_test
                ...
                st.warning(f"Error training TensorFlow Neural Network: {str(e)}")
...
def main():
    ...
    if uploaded_file is not None or 'df' in locals():
        ...
        if 'analyze_button' in locals() and analyze_button:
            ...
            # Store feature columns and target for later use
            analyzer.feature_columns = feature_cols
            analyzer.target_column = selected_target
            ...
            if not results_df.empty:
                ...
                # Performance visualizations - Organized Chart Dashboard
                st.subheader("📊 Performance Dashboard - Interactive Visualizations")
                
                # Create tabs for organized chart viewing
                chart_tab1, chart_tab2, chart_tab3 = st.tabs(["🏆 Rankings & Performance", "📈 Advanced Analytics", "🎯 Model Insights"])
                
                with chart_tab1:
                    st.markdown("### Algorithm Performance Rankings")
                    
                    # Row 1: Main Performance Charts
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        # 1. Top Algorithms Bar Chart (Optimized)
                        top_10 = results_display.head(10)
                        fig = px.bar(
                            top_10, 
                            y='Algorithm', 
                            x='Test R²',
                    ...
                    with col2:
                        # 2. Performance vs Speed Scatter (Optimized)
                        # Ensure positive size values
                        results_df_viz = results_df.copy()
                        results_df_viz['Size_RMSE'] = results_df_viz['Test RMSE'].abs() + 0.1  # Ensure positive
                        ...
                        fig = px.scatter(
                            ...
                            title="📈 Performance vs Training Speed",
                            size_max=15
                        )
                        fig.update_layout(
                            height=500,
                    ...
                    with col3:
                        ...
                        fig = px.box(
                            ...
                            x='Category', 
                            y='Test R²',
                            title="📦 Performance by Algorithm Category",
                            color='Category'
                        )
                        fig.update_layout(
                            height=400,
                    ...
                    with col4:
                        # 4. Overfitting Analysis (Improved)
                        # Ensure positive size values
                        results_df_viz2 = results_df.copy()
                        results_df_viz2['Size_Time'] = results_df_viz2['Training Time (s)'].abs() + 0.01  # Ensure positive
                        ...
                        fig = px.scatter(
                            ...
                            size='Size_Time',
                            hover_name='Algorithm',
                            title="⚠️ Training vs Test Performance",
                            color_continuous_scale='RdYlGn_r'
                        )
                        # Add perfect line
                        fig.add_shape(
                            type="line",
                            x0=results_df['Train R²'].min(),
                            y0=results_df['Train R²'].min(),
                            x1=results_df['Train R²'].max(),
                            y1=results_df['Train R²'].max(),
                ...
                with chart_tab2:
                    ...
                    with col1:
                        # 5. Multi-Metric Radar Chart (Top 5 Only for Performance)
                        top_5_models = results_display.head(5)
                        
                        # Create heatmap instead of radar for better performance
                        metrics_data = []
                        ...
                        fig = go.Figure(data=go.Heatmap(
                            z=metrics_data,
                            x=['R² Score', 'MAE', 'RMSE', 'Training Time'],
                            y=[row['Algorithm'] for _, row in top_5_models.iterrows()],
                            colorscale='RdYlGn',
                            showscale=True
                        ))
                        fig.update_layout(
                            title="🎯 Top 5 Models - Multi-Metric Heatmap",
                            height=400,
                            margin=dict(l=20, r=20, t=40, b=20)
                        )
                        st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col2:
                        # 6. Performance Distribution Violin Plot
                        fig = px.violin(
                            results_df, 
                            y='Test R²',
                            box=True,
                    ...
                    with col3:
                        # 7. Error Analysis
                        # Ensure positive size values for R² (can be negative)
                        results_df_viz3 = results_df.copy()
                        results_df_viz3['Size_R2'] = np.maximum(results_df_viz3['Test R²'], 0) + 0.01  # Ensure positive
                        ...
                        fig = px.scatter(
                            ...
                            size='Size_R2',
                            hover_name='Algorithm',
                            title="📉 Error Analysis (MAE vs RMSE)"
                        )
                        fig.update_layout(
                            height=400,
                            margin=dict(l=20, r=20, t=40, b=20)
                    ...
                    with col4:
                        ...
                        # Ensure positive values for efficiency calculation
                        efficiency_r2 = np.maximum(results_df['Test R²'], 0.001)  # Ensure positive R²
                        results_df['Efficiency'] = efficiency_r2 / (results_df['Training Time (s)'] + 0.001)
                        
                        # Create sorted dataframe with efficiency for display
                        efficiency_display = results_df.sort_values('Efficiency', ascending=False).head(10)
                        ...
                        fig = px.bar(
                            ...
                            y='Efficiency',
                            color='Efficiency',
                            title="⚡ Training Efficiency (R²/Time)",
                            color_continuous_scale='Viridis'
                        )
                        fig.update_layout(
                            height=400,
                ...
                with chart_tab3:
                    st.markdown("### Model Insights & Best Algorithm Analysis")
                    
                    # Best Model Analysis Section
                    best_model_name = best_model['Algorithm']
                    
                    if best_model_name in analyzer.trained_models:
                        ...
                        with col2:
                            # Feature importance or Residuals plot
                        ...
                        with col3:
                            ...
                            - **RMSE**: {best_model['Test RMSE']:.2f}
                            - **MAE**: {best_model['Test MAE']:.2f}
                            - **Training Time**: {best_model['Training Time (s)']:.3f}s
                            - **Overfitting**: {best_model['Overfitting']:.4f}
                            """)
                        
                        with col4:
                            # Quick Model Comparison (Top 3)
                            top_3 = results_display.head(3)
                            fig = px.bar(
                                top_3,
                                x='Test R²',
                                y='Algorithm',
                ...
                with col1:
                    ...
                    st.download_button(
                        ...
                        mime="text/csv",
                        help="Download complete algorithm comparison results"
                    )
                
                with col2:
                    # Create summary report
                    summary_report = f"""
SMART REGRESSION ANALYSIS REPORT
...
- Target Column: {analyzer.target_column or 'Unknown'}
...
TOP 5 ALGORITHMS:
{results_display[['Algorithm', 'Test R²', 'Test RMSE']].head().to_string(index=False)}

RECOMMENDATIONS:
✅ Use {best_model['Algorithm']} for best accuracy
✅ Category '{best_model['Category']}' works well for this dataset
✅ Model explains {best_model['Test R²']*100:.1f}% of target variance
"""
                    ...
                    st.download_button(
                        ...
                        mime="text/plain",
                        help="Download human-readable analysis summary"
                    )
                ...
                with col3:
                    ...
                    st.markdown(f"""
                    **🎯 Quick Insights:**
                    - Best: **{best_model['Algorithm']}**
                    - Accuracy: **{best_model['Test R²']:.1%}**
                    - Speed: **{best_model['Training Time (s)']:.2f}s**
                    - Error: **±{best_model['Test MAE']:.1f}**
                    """)
                ...
                with insights_col1:
                    ...
                    **Performance Metrics:**
                    - 📊 **R² Score**: {best_model['Test R²']:.4f} ({best_model['Test R²']*100:.1f}% variance explained)
                    - 📉 **RMSE**: {best_model['Test RMSE']:.2f}
                    - 📈 **MAE**: ±{best_model['Test MAE']:.2f} average error
                    - ⚡ **Speed**: {best_model['Training Time (s)']:.3f} seconds
                    - ⚠️ **Overfitting**: {best_model['Overfitting']:.4f}
                    """)
                ...
                with insights_col2:
                    ...
                    **Fastest Model**: {fastest_model['Algorithm']}  
                    Time: {fastest_model['Training Time (s)']:.3f}s
                    
                    **Dataset Difficulty**: {'Easy' if best_model['Test R²'] > 0.8 else 'Moderate' if best_model['Test R²'] > 0.6 else 'Challenging'}
                    
                    **Recommendation**: {'Excellent fit!' if best_model['Test R²'] > 0.8 else 'Good performance' if best_model['Test R²'] > 0.6 else 'Consider feature engineering'}
                    """)
